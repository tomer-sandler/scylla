...
ps -ef | grep scylla | grep cpuset &> /dev/null
if [ $? -ne 0 ]; then
	SCYLLA_SERVICE="1"
...
echo "Checking Scylla-JMX Service (nodetool) on Port $JMX_PORT"
echo "--------------------------------------------------"

nodetool -p$JMX_PORT status &> /dev/null
if [ $? -ne 0 ]; then
	JMX_SERVICE="1"
	echo "ERROR: Scylla-JMX is NOT Running / NOT Listening on Default Port"
...
for f in `sudo find /sys -name scheduler`; do echo -n "$f: "; cat  $f; done >> $OUTPUT_PATH1/io-sched-conf.txt
...
if [ "${SCYLLA_SERVICE}" == "1" ]; then
	...
	if [ $? -eq 0 ]; then
		...
		echo "ERROR: CQL is NOT Listening on Default Port"
		echo "Cannot collect data model info"
